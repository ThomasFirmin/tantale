use tantale::{algos::{mo::{CandidateSelector, NSGA2Selector}, utils::mo::{NonDominatedSorting, crowding_distance}}, core::{Accumulator, HasId, Lone, ParetoAccumulator}};
...
fn test_pareto_accumulator() {
    ...
    let ids = [5, 4, 3, 2, 1];
    assert!(front_ids.iter().all(|id| ids.contains(id)), "Pareto accumulator is wrong.");
}
...
fn test_nsga2_selection() {
    ...
    assert_eq!(ids, vec![5, 4, 3, 2, 1, 10, 9, 8, 6], "NSGA2 selection is wrong.");
}
...
fn test_non_dominated_arg_sorting() {
    ...
    let ids_f1 = fronts[0].iter().map(|&s| solutions[s].id().id).collect::<Vec<_>>();
    let ids_f2 = fronts[1].iter().map(|&s| solutions[s].id().id).collect::<Vec<_>>();
    let ids_f3 = fronts[2].iter().map(|&s| solutions[s].id().id).collect::<Vec<_>>();
    assert_eq!(ids_f1, vec![5, 4, 3, 2, 1]);
    assert_eq!(ids_f2, vec![10, 9, 8, 7, 6]);
    assert_eq!(ids_f3, vec![15, 14, 13, 12, 11]);
}
...
fn test_nsga2_argselection() {
    ...
    let ids = selected.iter().map(|s| solutions[*s].id().id).collect::<Vec<_>>();
    assert_eq!(ids, vec![5, 4, 3, 2, 1, 10, 9, 8, 6], "NSGA2 selection is wrong.");
}
