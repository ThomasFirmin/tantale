...
class MyOutcome:
    def __init__(self, obj1: float, obj2: float, info: float, samples: int, spiking: int):
        self.obj1 = obj1
        self.obj2 = obj2
        self.info = info
    ...
    @staticmethod
    def csv_header():
        return ["obj1", "obj2", "info", "samples", "spiking"]

    def csv_write(self):
        return [str(self.obj1), str(self.obj2), str(self.info), str(self.samples), str(self.spiking)]
...
def objective(x: list) -> MyOutcome:
    ...
    samples = 100
    spiking = 50
    return MyOutcome(obj1, obj2, info, samples, spiking)