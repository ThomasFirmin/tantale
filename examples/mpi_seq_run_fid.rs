...
fn main() {
    ...
    if proc.rank == 0 {
        drop(Cleaner("tmp_test_mpi_seq_run_fid".into()));
        let _clean = Cleaner("tmp_test_mpi_seq_run_fid".into());
    }
    ...
    let config = FolderConfig::new("tmp_test_mpi_seq_run_fid").init(&proc);
