...
use crate::{
    ...
    checkpointer::{CheckpointError, Checkpointer, DistCheckpointer, WorkerCheckpointer},
    experiment::{Evaluate, mpi::worker::WorkerState},
    optimizer::OptState,
    stop::Stop,
};
...
impl DistCheckpointer for MessagePack
{
    ...
    fn save_state_dist<OState:OptState, St:Stop, Eval:Evaluate>(&self, state: &OState, stop: &St, eval: &Eval, _rank:Rank) {
        let mut wrt = File::create(&self.path_optim).unwrap();
        rmp_serde::encode::write(&mut wrt, state).unwrap();
        let mut wrt = File::create(&self.path_stop).unwrap();
        rmp_serde::encode::write(&mut wrt, stop).unwrap();
    ...
    fn load_optimizer_dist<OState:OptState>(&self, _rank:Rank) -> Result<OState, CheckpointError> {
        // Check if file exist
        if self.config.path_check.try_exists().unwrap(){
            if self.path_optim.is_file() {
                let rdr = File::open(&self.path_optim).unwrap();
...
impl WCheckMessagePack
{
    pub fn new(proc: &MPIProcess, config:Arc<FolderConfig>)-> Self{
        ...
        WCheckMessagePack {config,path_worker}
    }
}
    ...
    fn after_load(&mut self, proc: &MPIProcess) {
        ...
        if self.config.is_dist{
            ...
            if !self.path_worker.try_exists().unwrap(){
                panic!("The Worker folder {} in checkpoint does not exists.", self.path_worker.display())
            }
        }else { panic!("The FolderConfig should be set for Distribued environment. Use the `.to_dist()` method.") }
    }
