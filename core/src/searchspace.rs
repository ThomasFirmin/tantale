    ...
    POpt: Partial<Opt,SInfo>,
{
    ...
    ///
    /// ```
    fn sample_obj(&self, rng: &mut ThreadRng, pid: u32) -> PObj;
    ...
    ///
    /// ```
    fn sample_opt(&self, rng: &mut ThreadRng, pid: u32) -> POpt;
    ...
    ///
    /// ```
    fn vec_onto_obj<T>(&self,inp: &[POpt]) -> Vec<PObj>;
    ...
    /// ```
    ///
    fn vec_sample_obj(&self,rng: &mut ThreadRng,pid: u32,size: usize) -> Vec<PObj>;
    ...
    ///
    /// ```
    fn vec_sample_opt(&self,rng: &mut ThreadRng,pid: u32,size: usize) -> Vec<POpt>;
    ...
    POpt: Partial<Opt,SInfo>,
{
    ...
    fn par_sample_obj(&self, pid: u32) -> PObj;
    /// Parallel version of [`sample_opt`](Searchspace::sample_opt) using [rayon].
    fn par_sample_opt(&self, pid: u32) -> POpt;
    ...
    fn par_vec_sample_obj(&self, pid: u32, size: usize) -> Vec<PObj>;
    /// Parallel version of [`sample_vec_opt`](Searchspace::sample_vec_opt) using [rayon].
    fn par_vec_sample_opt(&self, pid: u32, size: usize) -> Vec<POpt>;
    ...
    SInfo: SolInfo,
{
        ...
        SOpt : Solution<Opt,SInfo>;
    {
        ...
        out.x
            ...
            .zip(&inp.x)
            .zip(&self.variables)
            .for_each(|((o, i), v)| *o = v.onto_obj(i).unwrap());
        out
    }
    ...
    fn onto_opt(
        ...
        let mut out = inp.twin(Solution::<Opt, Cod, Out, SInfo, N>::default_x());
        out.x
            .iter_mut()
            .zip(&inp.x)
            .zip(&self.variables)
            .for_each(|((o, i), v)| *o = v.onto_opt(i).unwrap());
        out
    }
    ...
    fn sample_obj(&self, rng: &mut ThreadRng, pid: u32) -> Solution<Obj, Cod, Out, SInfo, N> {
        ...
        out.x
            .iter_mut()
            .zip(&self.variables)
            .for_each(|(inp, var)| *inp = var.sample_obj(rng));
        out
    }
    ...
    fn sample_opt(&self, rng: &mut ThreadRng, pid: u32) -> Solution<Opt, Cod, Out, SInfo, N> {
        ...
        out.x
            .iter_mut()
            .zip(&self.variables)
            .for_each(|(inp, var)| *inp = var.sample_opt(rng));
        out
    }
...
#[cfg(feature = "par")]
impl<Obj, Opt, Cod, Out, SInfo, const N: usize> ParSearchspace<Obj, Opt, Cod, Out, SInfo, N>
    for Sp<Obj, Opt, N>
where
    ...
    SInfo: SolInfo + Send + Sync,
{
    ...
    fn par_sample_obj(&self, pid: u32) -> Solution<Obj, Cod, Out, SInfo, N> {
        ...
        let outx = variter
            ...
            .collect();
        Solution::new(pid, outx, None, None)
    }
