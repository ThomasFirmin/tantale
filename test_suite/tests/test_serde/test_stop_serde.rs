...
fn test_calls_json(){
    ...
    calls.update(ExpStep::Evaluation);
    ...
    assert_eq!(calls.1,ncalls.1,"Serde mismatch on loaded Calls threshold");
    ncalls.update(ExpStep::Evaluation);
    assert_eq!(calls.0,2,"Serde on Calls wrong #calls after loading and updating");
}