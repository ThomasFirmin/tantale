use tantale_core::{EmptyInfo, ObjBase, SId, Searchspace, SingleCodomain, Solution, experiment::{Evaluate, sequential::seqevaluator::Evaluator}, solution::id, stop::Calls};
...
fn test_seq_evaluator() {
    ...
    cobj.iter().zip(sobj.iter()).zip(&linked).zip(copt.iter()).zip(sopt.iter()).for_each(
        |((((c,s),l),x),y)|
        {
            let cid = c.get_id().id;
            let sid = s.get_id().id;
    ...
    assert_eq!(hcobj.len(), 20, "Some IDs might be duplicated. Number of solutions is wrong for hcobj");
    assert_eq!(hsobj.len(), 20, "Some IDs might be duplicated. Number of solutions is wrong for hsobj");
    assert_eq!(hcopt.len(), 20, "Some IDs might be duplicated. Number of solutions is wrong for hcopt");
    assert_eq!(hsopt.len(), 20, "Some IDs might be duplicated. Number of solutions is wrong for hsopt");
    assert_eq!(hlink.len(), 20, "Some IDs might be duplicated. Number of solutions is wrong for hlink");
