...
use tantale::core::{Searchspace, ParSearchspace, Solution, SingleCodomain, HashOut, EmptyInfo};
    ...
    ($($name : ident),+) => {
        $(
            ...
            fn [<$name _single>]() {
                ...
                let sample_obj : Solution<_,SingleCodomain<HashOut>, HashOut,EmptyInfo,4> = sp.sample_obj(&mut rng, pid);
                let converted_opt = sp.onto_opt(&sample_obj);

                let sample_opt : Solution<_,SingleCodomain<HashOut>, HashOut,EmptyInfo,4> = sp.sample_opt(&mut rng, pid);
            ...
            fn [<$name _vec>]() {
                ...
                let vec_sample_obj : Vec<Solution<_,SingleCodomain<HashOut>, HashOut,EmptyInfo,4>> = sp.vec_sample_obj(&mut rng, pid, 3);
                let vec_converted_opt = sp.vec_onto_opt(&vec_sample_obj);

                let vec_sample_opt : Vec<Solution<_,SingleCodomain<HashOut>, HashOut,EmptyInfo,4>> = sp.vec_sample_opt(&mut rng, pid, 3);
            ...
            fn [<$name par_single>]() {
                ...
                let sample_obj : Solution<_,SingleCodomain<HashOut>, HashOut, EmptyInfo, 4> = sp.par_sample_obj(pid);
                let converted_opt = sp.par_onto_opt(&sample_obj);

                let sample_opt : Solution<_,SingleCodomain<HashOut>, HashOut, EmptyInfo, 4> = sp.par_sample_opt(pid);
            ...
            fn [<$name par_vec>]() {
                ...
                let vec_sample_obj : Vec<Solution<_, SingleCodomain<HashOut>,HashOut,EmptyInfo,4>> = sp.par_vec_sample_obj(pid, 3);
                let vec_converted_opt = sp.par_vec_onto_opt(&vec_sample_obj);

                let vec_sample_opt : Vec<Solution<_, SingleCodomain<HashOut>,HashOut,EmptyInfo,4>> = sp.par_vec_sample_opt(pid, 3);
